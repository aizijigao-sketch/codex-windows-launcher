param(
    [ValidateSet('official', 'thirdparty', 'thirdparty-preserve-auth', 'thirdparty-pure', 'repair', 'check', 'doctor', 'bootstrap', 'menu')]
    [string]$Mode = 'menu',
...
$Script:LauncherVersion = 'v0.4.33'
...
$Script:CodexElectronDataDir = Join-Path $env:APPDATA 'Codex'
$Script:CodexElectronStateDirs = @(
    'Local Storage',
    'Session Storage',
    'Network',
    'Cache',
...
$Script:RepairMarkerPath = Join-Path $Script:LauncherStateDir 'last-repair.json'
...
function Start-LaunchTarget {
    ...
    Test-CodexLaunchHealth -Target $Target | Out-Null
...
function Get-HistorySyncRemainingWork {
    ...
    $hardRemaining = 0
    $sawSpecificHardField = $false
    foreach ($field in @('provider_movable_threads', 'model_movable_threads', 'missing_user_event_threads', 'archived_threads', 'missing_session_index_entries', 'archived_index_mismatch_threads')) {
        $value = Get-ObjectValue -Object $status -Key $field
        if ($null -ne $value) {
            $sawSpecificHardField = $true
            $hardRemaining += [int]$value
        }
    }
    if (-not $sawSpecificHardField) {
        $value = Get-ObjectValue -Object $status -Key 'movable_threads'
        if ($null -ne $value) {
            $hardRemaining = [int]$value
        }
    }
    $sessionMetaRemaining = 0
    $sessionMetaValue = Get-ObjectValue -Object $status -Key 'movable_session_meta_entries'
    if ($null -ne $sessionMetaValue) {
        $sessionMetaRemaining = [int]$sessionMetaValue
    }
    $cwdPrefixRemaining = 0
    $cwdPrefixValue = Get-ObjectValue -Object $status -Key 'cwd_prefix_threads'
    if ($null -ne $cwdPrefixValue) {
        $cwdPrefixRemaining = [int]$cwdPrefixValue
    }

    return @{
        Remaining = $hardRemaining
        HardRemaining = $hardRemaining
        SessionMetaRemaining = $sessionMetaRemaining
        CwdPrefixRemaining = $cwdPrefixRemaining
...
function Test-HistorySyncProviderMatchesExpected {
    param(
        $Payload,
    ...
    $status = Get-HistorySyncStatusObject -Payload $Payload
    $currentProvider = Get-ObjectValue -Object $status -Key 'current_provider'
    if ($currentProvider -eq $ExpectedProvider) {
        return $true
    }

    $loginMode = Get-ObjectValue -Object (Get-ObjectValue -Object $status -Key 'login_mode') -Key 'mode'
    if ($ExpectedProvider -eq 'custom' -and $currentProvider -eq 'openai' -and $loginMode -eq 'cc-switch-local-route' -and (Test-ActiveConfigLooksThirdPartyRoute)) {
        Write-Warn '旧版 Codex History Sync Tool 将 CCSwitch/custom 路由识别为 provider=openai；已根据当前 config.toml 和 login_mode=cc-switch-local-route 视为第三方路由。'
        return $true
    }
...
function Test-HistorySyncExpectedProviderUnsupported {
    param(
        [int]$ExitCode,
        [string]$Raw,
        [string[]]$ProviderArgs
    ...
    if ($ExitCode -eq 0 -or -not $ProviderArgs -or $ProviderArgs.Count -eq 0) {
        return $false
    }
    ...
    return (
        $Raw -match '(?i)--expected-provider' -or
        $Raw -match '(?i)unrecognized arguments' -or
        $Raw -match '(?i)usage:\s*sync_backend\.py'
    )
}

function Test-HistorySyncCommandUnsupported {
    param(
        [int]$ExitCode,
        [string]$Raw
    )

    if ($ExitCode -eq 0 -or [string]::IsNullOrWhiteSpace($Raw)) {
        return $false
    }

    return (
        $Raw -match '(?i)usage:\s*sync_backend\.py' -or
        $Raw -match '(?i)unrecognized arguments:\s*(status|sync)' -or
        $Raw -match '(?i)invalid choice:\s*''?(status|sync)'
    )
}

function Invoke-HistorySyncBackendRawCommand {
    param(
        [string]$BackendPath,
        [string]$CodexHome,
        [string[]]$ProviderArgs,
    ...
    try {
        ...
        $completed = & py -3 $BackendPath --codex-home $CodexHome --json @ProviderArgs $Command 2>&1
...
function Invoke-HistorySyncBackendCommand {
    param(
        [string]$BackendPath,
        [string]$CodexHome,
        [string[]]$ProviderArgs,
    ...
    $usedProviderArgs = @($ProviderArgs)
    $result = Invoke-HistorySyncBackendRawCommand -BackendPath $BackendPath -CodexHome $CodexHome -ProviderArgs $usedProviderArgs -Command $Command
    $exitCode = $result.ExitCode
    $raw = $result.Raw
    $fallbackUsed = $false

    if (Test-HistorySyncExpectedProviderUnsupported -ExitCode $exitCode -Raw $raw -ProviderArgs $usedProviderArgs) {
        $fallbackUsed = $true
        $usedProviderArgs = @()
        $result = Invoke-HistorySyncBackendRawCommand -BackendPath $BackendPath -CodexHome $CodexHome -ProviderArgs $usedProviderArgs -Command $Command
        $exitCode = $result.ExitCode
    ...
    $commandUnsupported = Test-HistorySyncCommandUnsupported -ExitCode $exitCode -Raw $raw

    return New-Object PSObject -Property @{
        ExitCode = $exitCode
        Raw = $raw
        ProviderArgs = $usedProviderArgs
        ProviderArgText = if ($usedProviderArgs.Count -gt 0) { " $($usedProviderArgs -join ' ')" } else { '' }
        ExpectedProviderUnsupported = $fallbackUsed
...
function Invoke-HistorySyncBeforeCodexLaunch {
    ...
    try {
        ...
        $expectedProviderUnsupported = $false
        for ($attempt = 1; $attempt -le 3; $attempt++) {
            Write-Info "执行历史状态检查($attempt/3)：py -3 $backendPath --codex-home $Script:DefaultCodexHome --json$providerArgText status"
            $statusResult = Invoke-HistorySyncBackendCommand -BackendPath $backendPath -CodexHome $Script:DefaultCodexHome -ProviderArgs $historyProviderArgs -Command 'status'
            if ($statusResult.ExpectedProviderUnsupported) {
                $expectedProviderUnsupported = $true
                $historyProviderArgs = @($statusResult.ProviderArgs)
                $providerArgText = $statusResult.ProviderArgText
                Write-Warn '旧版 Codex History Sync Tool 不支持 --expected-provider；已降级为旧参数调用，provider 仍由启动器本地校验。'
            }
            if ($statusResult.CommandUnsupported) {
                Write-Warn '当前 Codex History Sync Tool 版本不支持启动器需要的 status/sync 命令；本次跳过历史检查并继续启动 Codex。'
                Write-Next '建议升级 codex-history-sync-tool-windows；本次不会复制凭据或聊天，只会继续启动 Codex。'
                return $true
            }
            Write-Info "历史状态检查($attempt/3)退出码：$($statusResult.ExitCode)"
            ...
            if (Test-HistorySyncProviderMatchesExpected -Payload $statusPayload -ExpectedProvider $ExpectedProvider) {
                $providerMismatch = $false
                break
            }
        ...
        Write-Info "历史同步目标 provider=$currentProvider，总线程=$totalThreads，硬异常=$($precheck.HardRemaining)，cwd前缀软异常=$($precheck.CwdPrefixRemaining)，session_meta软异常=$($precheck.SessionMetaRemaining)。"
        Write-Info "历史状态摘要 before：$(Format-HistoryStatusSummary -Payload $statusPayload)"
        if ($providerMismatch -or (-not (Test-HistorySyncProviderMatchesExpected -Payload $statusPayload -ExpectedProvider $ExpectedProvider))) {
        ...
        if ($expectedProviderUnsupported -and -not [string]::IsNullOrWhiteSpace($ExpectedProvider) -and $currentProvider -ne $ExpectedProvider -and $precheck.Remaining -gt 0) {
            Write-ErrorLine "当前 Codex History Sync Tool 版本不支持 --expected-provider $ExpectedProvider，且检测到待修复聊天异常；为避免把聊天记录修到 $currentProvider 通道，本次不会执行历史恢复，也不会启动 Codex。"
            Write-Next '请先升级 codex-history-sync-tool-windows 到支持 --expected-provider 的版本，然后重新运行启动器菜单。'
            return $false
        }

        if ($precheck.Remaining -le 0 -and -not $providerMismatch) {
            if ($precheck.SessionMetaRemaining -gt 0) {
                Write-Warn "历史列表状态已干净：总线程=$totalThreads，硬异常=0；仅剩 cwd前缀软异常=$($precheck.CwdPrefixRemaining)、session_meta软异常=$($precheck.SessionMetaRemaining)，不再反复等待修复。"
                return $true
            }
            if ($precheck.CwdPrefixRemaining -gt 0) {
                Write-Warn "历史列表状态已干净：总线程=$totalThreads，硬异常=0；仅剩 cwd前缀软异常=$($precheck.CwdPrefixRemaining)，这是 Codex Desktop 常见长路径写法，不再反复等待修复。"
                return $true
            }
            Write-Ok "历史状态已干净：总线程=$totalThreads，硬异常=0；跳过修复。"
            return $true
        }

        Write-Warn "检测到聊天记录硬异常：待修复=$($precheck.Remaining)，provider异常=$providerMismatch。"
        ...
        $syncResult = Invoke-HistorySyncBackendCommand -BackendPath $backendPath -CodexHome $Script:DefaultCodexHome -ProviderArgs $historyProviderArgs -Command 'sync'
        if ($syncResult.ExpectedProviderUnsupported) {
            $historyProviderArgs = @($syncResult.ProviderArgs)
            $providerArgText = $syncResult.ProviderArgText
            Write-Warn '旧版 Codex History Sync Tool 不支持 --expected-provider；定向恢复已降级为旧参数调用。'
        }
        if ($syncResult.CommandUnsupported) {
            Write-Warn '当前 Codex History Sync Tool 版本不支持启动器需要的 status/sync 命令；本次跳过历史检查并继续启动 Codex。'
            Write-Next '建议升级 codex-history-sync-tool-windows；本次不会复制凭据或聊天，只会继续启动 Codex。'
            return $true
        }
        Write-Info "定向历史恢复退出码：$($syncResult.ExitCode)"
        $raw = $syncResult.Raw
        if ([string]::IsNullOrWhiteSpace($raw)) {
        ...
        $postResult = Invoke-HistorySyncBackendCommand -BackendPath $backendPath -CodexHome $Script:DefaultCodexHome -ProviderArgs $historyProviderArgs -Command 'status'
        if ($postResult.ExpectedProviderUnsupported) {
            $historyProviderArgs = @($postResult.ProviderArgs)
            $providerArgText = $postResult.ProviderArgText
            Write-Warn '旧版 Codex History Sync Tool 不支持 --expected-provider；恢复后状态检查已降级为旧参数调用。'
        }
        if ($postResult.CommandUnsupported) {
            Write-Warn '当前 Codex History Sync Tool 版本不支持启动器需要的 status/sync 命令；本次跳过历史检查并继续启动 Codex。'
            Write-Next '建议升级 codex-history-sync-tool-windows；本次不会复制凭据或聊天，只会继续启动 Codex。'
            return $true
        }
        Write-Info "恢复后历史状态检查退出码：$($postResult.ExitCode)"
        $postRaw = $postResult.Raw
        if (-not [string]::IsNullOrWhiteSpace($postRaw)) {
            ...
            Write-Info "定向恢复后历史状态：provider=$postProvider，总线程=$postTotal，硬异常剩余=$($postWork.HardRemaining)，cwd前缀软异常=$($postWork.CwdPrefixRemaining)，session_meta软异常=$($postWork.SessionMetaRemaining)。"
            Write-Info "历史状态摘要 after：$(Format-HistoryStatusSummary -Payload $postPayload)"
            if (-not (Test-HistorySyncProviderMatchesExpected -Payload $postPayload -ExpectedProvider $ExpectedProvider)) {
            ...
            if ($postWork.Remaining -le 0) {
                if ($postWork.SessionMetaRemaining -gt 0) {
                    Write-Warn "Codex History Sync Tool 定向恢复完成，聊天列表状态已干净；仅剩 cwd前缀软异常=$($postWork.CwdPrefixRemaining)、session_meta软异常=$($postWork.SessionMetaRemaining)，不阻止启动。"
                    return $true
                }
                if ($postWork.CwdPrefixRemaining -gt 0) {
                    Write-Warn "Codex History Sync Tool 定向恢复完成，聊天列表状态已干净；仅剩 cwd前缀软异常=$($postWork.CwdPrefixRemaining)，不阻止启动。"
                    return $true
                }
...
function Test-PathInsideDirectory {
    ...
    try {
        ...
        return ($resolvedPath -eq $resolvedParent -or $resolvedPath.StartsWith($resolvedParent + '\', [System.StringComparison]::OrdinalIgnoreCase))
    } catch {
        return $false
    }
}

function Backup-CodexDesktopCrashState {
    param([switch]$NoWrite)

    $timestamp = Get-Date -Format 'yyyyMMdd-HHmmss'
    $backupRoot = Join-Path $Script:BackupDir "codex-desktop-crash-state.$timestamp"

    if ($NoWrite) {
        Write-Info "NoLaunch: 将会备份 Codex Desktop 崩溃缓存到：$backupRoot"
        return $backupRoot
    }

    Ensure-Directory -Path $backupRoot

    foreach ($name in $Script:CodexElectronStateDirs) {
        $source = Join-Path $Script:CodexElectronDataDir $name
        if (-not (Test-Path -LiteralPath $source -PathType Container)) {
            continue
        }
        if (-not (Test-PathInsideDirectory -Path $source -Parent $Script:CodexElectronDataDir)) {
            Write-Warn "跳过不在 Codex Electron 数据目录内的路径：$source"
            continue
        }
    ...
    if (Test-Path -LiteralPath $Script:CodexGlobalStatePath -PathType Leaf) {
        Copy-Item -LiteralPath $Script:CodexGlobalStatePath -Destination (Join-Path $backupRoot '.codex-global-state.json') -Force
    }

    Write-Info "已备份 Codex Desktop 崩溃缓存：$backupRoot"
    return $backupRoot
}

function Clear-CodexDesktopCrashState {
    param([switch]$NoWrite)

    foreach ($name in $Script:CodexElectronStateDirs) {
        $target = Join-Path $Script:CodexElectronDataDir $name
        if (-not (Test-Path -LiteralPath $target -PathType Container)) {
            continue
        }
        if (-not (Test-PathInsideDirectory -Path $target -Parent $Script:CodexElectronDataDir)) {
            Write-Warn "跳过不在 Codex Electron 数据目录内的路径：$target"
            continue
        }
        if ($NoWrite) {
            Write-Info "NoLaunch: 将会清理 Codex Desktop 缓存目录：$target"
            continue
        }
    ...
    if (Test-Path -LiteralPath $Script:CodexGlobalStatePath -PathType Leaf) {
        if ($NoWrite) {
            Write-Info "NoLaunch: 将会清理 Codex UI 状态文件：$Script:CodexGlobalStatePath"
        } else {
            Remove-Item -LiteralPath $Script:CodexGlobalStatePath -Force
        }
    }

    Write-Ok '已备份并清理 Codex Desktop 崩溃缓存。'
    return $true
}

function Export-CodexDesktopRepairEvidence {
    param(
    ...
    $evidencePath = Join-Path $DestinationDir 'repair-evidence.txt'
    $lines = New-Object System.Collections.Generic.List[string]
    $lines.Add("Codex Windows Launcher repair evidence")
    $lines.Add("time=$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')")
    $lines.Add("launcher_version=$Script:LauncherVersion")
    $lines.Add("codex_electron_data_dir=$Script:CodexElectronDataDir")
    $lines.Add("codex_global_state_exists=$(Test-Path -LiteralPath $Script:CodexGlobalStatePath -PathType Leaf)")

    if (Test-Path -LiteralPath $Script:CodexGlobalStatePath -PathType Leaf) {
        $globalStateItem = Get-Item -LiteralPath $Script:CodexGlobalStatePath
        $lines.Add("codex_global_state_last_write=$($globalStateItem.LastWriteTime.ToString('s'))")
        $lines.Add("codex_global_state_length=$($globalStateItem.Length)")
    }

    if (Test-Path -LiteralPath $Script:CodexElectronDataDir -PathType Container) {
        $lines.Add('electron_state_dirs:')
        foreach ($item in @(Get-ChildItem -LiteralPath $Script:CodexElectronDataDir -ErrorAction SilentlyContinue | Where-Object { $_.PSIsContainer })) {
            if ($item.Name -in $Script:CodexElectronStateDirs) {
                $lines.Add(("  {0}; last_write={1}" -f $item.Name, $item.LastWriteTime.ToString('s')))
            }
        }
    } else {
        $lines.Add('electron_state_dirs=<missing>')
    }

    $runningCodex = @(Get-ProcessesByNames -Names $Script:CodexProcessNames)
    $lines.Add("codex_process_count=$($runningCodex.Count)")
    foreach ($proc in $runningCodex | Select-Object -First 5) {
        $lines.Add(("  process name={0}; pid={1}; path={2}" -f $proc.Name, $proc.ProcessId, $proc.ExecutablePath))
    }

    try {
        $events = @(Get-WinEvent -FilterHashtable @{ LogName = 'Application'; StartTime = (Get-Date).AddHours(-6) } -ErrorAction SilentlyContinue |
            Where-Object { $_.ProviderName -match 'Application Error|Windows Error Reporting|Codex|OpenAI' -or $_.Message -match 'Codex|OpenAI' } |
            Select-Object -First 10)
        $lines.Add("recent_windows_application_events=$($events.Count)")
        foreach ($event in $events) {
            $message = Protect-LogMessage -Message (($event.Message -replace '\s+', ' ').Trim())
            if ($message.Length -gt 260) {
                $message = $message.Substring(0, 260)
            }
            $lines.Add(("  {0}; {1}; id={2}; {3}" -f $event.TimeCreated.ToString('s'), $event.ProviderName, $event.Id, $message))
        }
    } catch {
        $lines.Add("recent_windows_application_events_error=$($_.Exception.Message)")
    }

    if ($NoWrite) {
        Write-Info "NoLaunch: 将会保存 Codex Desktop 修复证据：$evidencePath"
        return $evidencePath
    }

    Ensure-Directory -Path $DestinationDir
    Set-Content -LiteralPath $evidencePath -Value $lines.ToArray() -Encoding UTF8

    if (Test-Path -LiteralPath $Script:LogDir -PathType Container) {
        $latestLogs = @(Get-ChildItem -LiteralPath $Script:LogDir -Filter 'launcher.*.log' -File -ErrorAction SilentlyContinue |
            Sort-Object LastWriteTime -Descending |
            Select-Object -First 5)
        $index = 1
        foreach ($log in $latestLogs) {
            $destName = "launcher-log-$index-$($log.Name)"
            Copy-Item -LiteralPath $log.FullName -Destination (Join-Path $DestinationDir $destName) -Force
            $index++
        }
    }

    Write-Info "已保存 Codex Desktop 修复证据：$evidencePath"
    return $evidencePath
}

function Get-CodexDesktopLogRoots {
    $roots = New-Object System.Collections.Generic.List[string]

    $packageRoot = Join-Path $env:LOCALAPPDATA 'Packages'
    if (Test-Path -LiteralPath $packageRoot -PathType Container) {
        foreach ($package in @(Get-ChildItem -LiteralPath $packageRoot -Directory -Filter 'OpenAI.Codex_*' -ErrorAction SilentlyContinue)) {
            $logs = Join-Path $package.FullName 'LocalCache\Local\Codex\Logs'
    ...
    foreach ($candidate in @(
        ...
        (Join-Path $Script:CodexElectronDataDir 'Logs')
    )) {
    ...
    @($roots.ToArray() | Select-Object -Unique)
}

function Export-CodexDesktopLaunchEvidence {
    param(
        [string]$Reason,
        [switch]$NoWrite
    )
    ...
    $timestamp = Get-Date -Format 'yyyyMMdd-HHmmss'
    $destinationDir = Join-Path $Script:BackupDir "launch-evidence.$timestamp"
    $evidencePath = Join-Path $destinationDir 'launch-evidence.txt'

    if ($NoWrite) {
        Write-Info "NoLaunch: 将会保存 Codex Desktop 启动诊断证据：$evidencePath"
    ...
    $lines = New-Object System.Collections.Generic.List[string]
    $lines.Add('Codex Windows Launcher launch evidence')
    $lines.Add("time=$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')")
    $lines.Add("launcher_version=$Script:LauncherVersion")
    $lines.Add("reason=$Reason")
    $lines.Add("run_log=$Script:RunLogPath")
    $lines.Add("codex_home=$Script:DefaultCodexHome")
    $lines.Add("codex_electron_data_dir=$Script:CodexElectronDataDir")
    $lines.Add('ui_error_hint=If Codex shows "Oops, an error has occurred" with buttons like "Update Codex" or "Try again", send this launch-evidence folder.')

    $summary = Get-ActiveConfigProviderSummary
    $lines.Add("config_summary_exists=$($summary.Exists); provider=$($summary.ModelProvider); model=$($summary.Model); custom_section=$($summary.HasCustomProviderSection); local_route=$($summary.HasLocalRouteBaseUrl); requires_openai_auth=$($summary.RequiresOpenAIAuth)")

    $runningCodex = @(Get-ProcessesByNames -Names $Script:CodexProcessNames)
    $lines.Add("codex_process_count=$($runningCodex.Count)")
    foreach ($proc in $runningCodex | Select-Object -First 8) {
        $lines.Add(("  process name={0}; pid={1}; path={2}" -f $proc.Name, $proc.ProcessId, $proc.ExecutablePath))
    }
    ...
    $logRoots = @(Get-CodexDesktopLogRoots)
    $lines.Add("codex_desktop_log_roots=$($logRoots.Count)")
    ...
    $copied = 0
    foreach ($root in $logRoots) {
        $latestLogs = @(Get-ChildItem -LiteralPath $root -Recurse -File -ErrorAction SilentlyContinue |
            Where-Object { $_.Name -match '\.(log|txt)$' -and $_.Length -le 5MB } |
            Sort-Object LastWriteTime -Descending |
            Select-Object -First 8)
        foreach ($log in $latestLogs) {
            $copied++
            $safeName = ($log.FullName -replace '^[A-Za-z]:\\', '' -replace '[\\/:*?"<>| ]+', '_')
            $destName = "codex-desktop-log-$copied-$safeName"
            Copy-Item -LiteralPath $log.FullName -Destination (Join-Path $destinationDir $destName) -Force
            $lines.Add(("copied_codex_log={0}; last_write={1}; length={2}" -f $log.FullName, $log.LastWriteTime.ToString('s'), $log.Length))
        }
    }

    if (Test-Path -LiteralPath $Script:RunLogPath -PathType Leaf) {
        Copy-Item -LiteralPath $Script:RunLogPath -Destination (Join-Path $destinationDir ('launcher-log-' + (Split-Path -Leaf $Script:RunLogPath))) -Force
    }

    Set-Content -LiteralPath $evidencePath -Value $lines.ToArray() -Encoding UTF8
    Write-Warn "已保存 Codex Desktop 启动诊断证据：$destinationDir"
    return $destinationDir
}

function Test-CodexDesktopRepairRecommended {
    param([int]$RecentMinutes = 30)

    $since = (Get-Date).AddMinutes(-1 * [Math]::Max($RecentMinutes, 1))
    if (Test-Path -LiteralPath $Script:CodexGlobalStatePath -PathType Leaf) {
        $stateItem = Get-Item -LiteralPath $Script:CodexGlobalStatePath
        if ($stateItem.LastWriteTime -ge $since) {
            return $true
        }
    }

    if (Test-Path -LiteralPath $Script:CodexElectronDataDir -PathType Container) {
        $recentStateDirs = @(Get-ChildItem -LiteralPath $Script:CodexElectronDataDir -ErrorAction SilentlyContinue |
            Where-Object { $_.PSIsContainer -and $_.Name -in $Script:CodexElectronStateDirs -and $_.LastWriteTime -ge $since })
        if ($recentStateDirs.Count -gt 0) {
            return $true
        }
    }
...
function Write-RepairMarker {
    ...
    if ($NoWrite) {
        Write-Info "NoLaunch: 将会写入 repair 标记：$Script:RepairMarkerPath"
        return
    }
    ...
    Set-Content -LiteralPath $Script:RepairMarkerPath -Value $marker -Encoding UTF8
}

function Test-RecentDesktopRepair {
    param([int]$RecentMinutes = 30)

    if (-not (Test-Path -LiteralPath $Script:RepairMarkerPath -PathType Leaf)) {
        return $false
    }

    try {
        $marker = Get-Content -LiteralPath $Script:RepairMarkerPath -Raw -Encoding UTF8 | ConvertFrom-Json
        if (-not ($marker.PSObject.Properties.Name -contains 'repaired_at')) {
            return $false
        }
        $repairedAt = [datetime]::Parse([string]$marker.repaired_at)
        return ($repairedAt -ge (Get-Date).AddMinutes(-1 * [Math]::Max($RecentMinutes, 1)))
    } catch {
        return $false
    }
}

function Reset-LauncherUiSnapshotAfterRepair {
    param(
        [string]$EvidenceDir,
        [switch]$NoWrite
    )
    ...
    $timestamp = Get-Date -Format 'yyyyMMdd-HHmmss'
    $backupName = "codex-ui-state.$timestamp.before-repair.json"
    $backupPath = if (-not [string]::IsNullOrWhiteSpace($EvidenceDir)) {
        Join-Path $EvidenceDir $backupName
    } else {
        Join-Path $Script:BackupDir $backupName
    }

    if ($NoWrite) {
        Write-Info "NoLaunch: 将会隔离启动器保存的 Codex UI 快照：$Script:CodexUiStateSnapshotPath -> $backupPath"
        return
    }

    Ensure-Directory -Path (Split-Path -Parent $backupPath)
    Move-Item -LiteralPath $Script:CodexUiStateSnapshotPath -Destination $backupPath -Force
    Write-Info "已隔离启动器保存的 Codex UI 快照：$backupPath"
}

function Restore-CodexUiStateSnapshotUnlessRecentRepair {
    param([switch]$NoWrite)

    if (Test-RecentDesktopRepair -RecentMinutes 30) {
        Write-Warn '刚执行过 repair，跳过恢复旧 UI 快照，避免错误页状态复发。'
        return $true
    }

    Restore-CodexUiStateSnapshot -NoWrite:$NoWrite | Out-Null
    return $true
}

function Write-CodexDesktopRepairSuggestion {
    param(
        [string]$Reason,
        [string]$EvidenceDir
    )

    $repairCommandPath = Join-Path (Split-Path -Parent $PSCommandPath) 'codex-launcher.ps1'
    Write-Warn "Codex Desktop 可能进入错误页或启动后立即退出：$Reason"
    Write-Next '如果界面显示 "Oops, an error has occurred" / "Update Codex" / "Try again"，请先运行 repair，再把启动诊断证据发回。'
    if (-not [string]::IsNullOrWhiteSpace($EvidenceDir)) {
        Write-Next "启动诊断证据目录：$EvidenceDir"
    }
    Write-Next '建议运行修复模式；它会备份并清理 Electron/UI 崩溃缓存，不删除登录、配置和聊天。'
    Write-Next "powershell -NoProfile -ExecutionPolicy Bypass -File `"$repairCommandPath`" -Mode repair"
    Write-Next "请保留本次日志和 repair 生成的 repair-evidence.txt，便于后续升级修复：$Script:RunLogPath"
}

function Test-CodexLaunchHealth {
    param(
        $Target,
        [int]$WaitSeconds = 6
    )
    ...
    $running = $false
    if ($Target -and $Target.Kind -eq 'Exe') {
        $running = Test-ProcessRunning -Path $Target.Value -Names $Script:CodexProcessNames
    } else {
        $running = Test-ProcessRunning -Path '' -Names $Script:CodexProcessNames
    }

    if (-not $running) {
        $evidenceDir = Export-CodexDesktopLaunchEvidence -Reason "启动后 $WaitSeconds 秒内未检测到 Codex 进程。"
        Write-CodexDesktopRepairSuggestion -Reason "启动后 $WaitSeconds 秒内未检测到 Codex 进程。" -EvidenceDir $evidenceDir
        return $false
    }

    if (Test-CodexDesktopRepairRecommended -RecentMinutes 3) {
        $evidenceDir = Export-CodexDesktopLaunchEvidence -Reason 'Codex 进程仍在运行，但本地 Electron/UI 状态刚刚改动；如果当前界面是错误页，需要保留证据并修复。'
        Write-Warn '检测到 Codex Desktop 本地 UI 状态刚刚改动；如果当前界面显示“糟糕，出错了”或 "Oops, an error has occurred"，请运行 repair。'
        $repairCommandPath = Join-Path (Split-Path -Parent $PSCommandPath) 'codex-launcher.ps1'
        Write-Next "启动诊断证据目录：$evidenceDir"
        Write-Next "powershell -NoProfile -ExecutionPolicy Bypass -File `"$repairCommandPath`" -Mode repair"
    }
...
function Set-PreserveAuthCustomProviderRequiresOfficialAuth {
    ...
    if (-not (Test-ActiveConfigLooksThirdPartyRoute)) {
        Write-Warn '当前 config.toml 不像 CCSwitch 本地路由；跳过官方 OAuth UI 状态标记修正。'
        return $false
    }

    $lines = Read-ConfigLines -Path $Script:ActiveConfigPath
    if ($lines.Count -eq 0) {
        Write-Warn '当前 config.toml 为空；无法修正 custom provider 官方 OAuth UI 状态标记。'
        return $false
    }
    ...
    if (-not $sawCustomProvider) {
        Write-Warn '当前 config.toml 没有 [model_providers.custom]；无法标记第三方路由需要官方 OAuth UI 状态。'
        return $false
    }

    if (-not $changed) {
        Write-Ok '第三方路由已标记为需要官方 OAuth UI 状态。'
        return $true
    }
    ...
    Set-Content -LiteralPath $Script:ActiveConfigPath -Value $result.ToArray() -Encoding UTF8
    Write-Ok '已把第三方 custom provider 标记为需要官方 OAuth UI 状态。'
    return $true
}
...
function New-OfficialAuthBaselineForPreserveMode {
    param([switch]$NoWrite)

    if ((Get-AuthState -Path $Script:ActiveAuthPath) -ne 'official-like') {
        Write-Warn '菜单 2 开始时 active auth.json 不是可确认官方登录态；不会建立官方 auth 基准。'
        return $null
    }

    $hash = Get-FileSha12 -Path $Script:ActiveAuthPath
    $timestamp = Get-Date -Format 'yyyyMMdd-HHmmss'
    $baselinePath = Join-Path $Script:BackupDir "auth.json.$timestamp.preserve-auth-baseline.bak"

    if ($NoWrite) {
        Write-Info "NoLaunch: 将会记录菜单 2 官方 auth 基准 hash=$hash。"
        return New-Object PSObject -Property @{ Path = $Script:ActiveAuthPath; Hash = $hash }
    }

    Ensure-Directory -Path $Script:BackupDir
    Copy-Item -LiteralPath $Script:ActiveAuthPath -Destination $baselinePath -Force
    Write-Info "菜单 2 官方 auth 基准 hash=$hash；已保存基准备份：$baselinePath"
    return New-Object PSObject -Property @{ Path = $baselinePath; Hash = $hash }
}

function Restore-OfficialAuthBaselineForPreserveMode {
    param(
        $Baseline,
        [switch]$NoWrite
    )
    ...
    $baselineHash = [string]$Baseline.Hash
    if ([string]::IsNullOrWhiteSpace($baselineHash)) {
        return $false
    }

    if ($NoWrite) {
        Write-Info "NoLaunch: 如 auth 被切换，将恢复菜单 2 官方 auth 基准 hash=$baselineHash。"
        return $true
    }

    if (-not (Test-Path -LiteralPath $baselinePath -PathType Leaf)) {
        Write-Warn "菜单 2 官方 auth 基准文件不存在，无法恢复：$baselinePath"
        return $false
    }

    $actualBaselineHash = Get-FileSha12 -Path $baselinePath
    if ($actualBaselineHash -ne $baselineHash) {
        Write-Warn "菜单 2 官方 auth 基准 hash 不一致：expected=$baselineHash actual=$actualBaselineHash"
        return $false
    }

    $activeHash = Get-FileSha12 -Path $Script:ActiveAuthPath
    if ($activeHash -ne $baselineHash) {
        Backup-LauncherFile -Path $Script:ActiveAuthPath -Reason 'before-preserve-auth-baseline-restore' | Out-Null
        Copy-Item -LiteralPath $baselinePath -Destination $Script:ActiveAuthPath -Force
        Write-Warn "active auth.json 已从 $activeHash 恢复为菜单 2 基准 hash=$baselineHash。"
    }

    $officialAuthPath = Join-Path $Script:OfficialProfileDir 'auth.json'
    $officialHash = Get-FileSha12 -Path $officialAuthPath
    if ($officialHash -ne $baselineHash) {
        Ensure-Directory -Path $Script:OfficialProfileDir
        if (Test-Path -LiteralPath $officialAuthPath -PathType Leaf) {
            Backup-LauncherFile -Path $officialAuthPath -Reason 'before-preserve-auth-baseline-official-cache-restore' | Out-Null
        }
        Copy-Item -LiteralPath $baselinePath -Destination $officialAuthPath -Force
        Write-Warn "official profile auth.json 已从 $officialHash 恢复为菜单 2 基准 hash=$baselineHash。"
    }
...
function Test-OfficialProfileCacheNeedsUpdate {
    if (-not (Test-CurrentLooksOfficialProfile)) {
        return $false
    }

    $activeAuth = $Script:ActiveAuthPath
    $cachedAuth = Join-Path $Script:OfficialProfileDir 'auth.json'
    if (-not (Test-Path -LiteralPath $cachedAuth -PathType Leaf)) {
        return $true
    }
    if ((Get-Item -LiteralPath $activeAuth).LastWriteTime -gt (Get-Item -LiteralPath $cachedAuth).LastWriteTime) {
        return $true
    }

    $cachedConfig = Join-Path $Script:OfficialProfileDir 'config.toml'
    if ((Test-Path -LiteralPath $Script:ActiveConfigPath -PathType Leaf) -and -not (Test-Path -LiteralPath $cachedConfig -PathType Leaf)) {
        return $true
    }
...
function Save-OfficialProfileIfCurrentLooksOfficial {
    param(
        [switch]$NoWrite,
        [switch]$OnlyIfCacheMissingOrOlder
    )

    if (Test-CurrentLooksOfficialProfile) {
        if ($OnlyIfCacheMissingOrOlder -and -not (Test-OfficialProfileCacheNeedsUpdate)) {
            Write-Info '当前官方登录态已可用且缓存不旧；不重复保存官方 profile。'
            return $true
        }
        ...
        try {
            ...
            Write-Warn "保存官方 profile 失败；继续使用当前官方登录态启动：$($_.Exception.Message)"
            return $false
        }
    }
...
function Prepare-OfficialProfileForOfficialMode {
    param([switch]$NoWrite)

    if (Test-CurrentLooksOfficialProfile) {
        Write-Info '当前默认 .codex 已是官方登录态；跳过恢复旧 official profile。'
        Save-OfficialProfileIfCurrentLooksOfficial -OnlyIfCacheMissingOrOlder -NoWrite:$NoWrite | Out-Null
        return $true
    }

    Write-Info '当前默认 .codex 不是可确认的官方登录态；尝试恢复 official profile。'
    if (Restore-OfficialProfile -NoWrite:$NoWrite) {
        Write-Info '已找到官方状态缓存；将恢复它，不强制重新登录。'
        return $true
    }

    Write-Warn '没有已保存的官方状态。首次官方启动可能需要网页登录。'
    Repair-ActiveConfigForOfficial -NoWrite:$NoWrite
    Disable-ApiKeyAuthForOfficial -NoWrite:$NoWrite
    return $false
}
...
function Ensure-OfficialAuthForPreserveMode {
    ...
    if ($Baseline) {
        if (Restore-OfficialAuthBaselineForPreserveMode -Baseline $Baseline -NoWrite:$NoWrite) {
            return $true
        }
        Write-Warn '菜单 2 官方 auth 基准恢复失败；将退回普通 official profile 检查。'
    }
...
function Confirm-OfficialAuthForPreserveMode {
    ...
    if ($NoLaunch) {
        Write-Info 'NoLaunch: 将会确认最终 auth.json 仍是官方 ChatGPT/OAuth 登录态，并匹配菜单 2 auth 基准。'
        return $true
    }

    if ($Baseline -and -not (Restore-OfficialAuthBaselineForPreserveMode -Baseline $Baseline)) {
        Write-ErrorLine '最终 auth.json 无法恢复到菜单 2 开始时的官方 auth 基准；本次不会启动 Codex，避免再次要求登录。'
        return $false
    }
    ...
    if ($authState -eq 'official-like') {
        if ($Baseline) {
            ...
            $baselineHash = [string]$Baseline.Hash
            if ($activeHash -ne $baselineHash) {
                Write-ErrorLine "最终 auth.json hash=$activeHash，不匹配菜单 2 开始时的官方 auth 基准 hash=$baselineHash；本次不会启动 Codex。"
                return $false
            }
            Write-Ok "最终 auth.json 已确认是官方 ChatGPT/OAuth 登录态，且匹配菜单 2 基准 hash=$baselineHash。"
            return $true
        }
...
function Invoke-Doctor {
    ...
    if (Test-CodexDesktopRepairRecommended -RecentMinutes 60) {
        Write-CodexDesktopRepairSuggestion -Reason '检测到近期 Codex Desktop Electron/UI 状态变化；如果当前界面显示错误页，优先运行 repair。'
    }
...
function Start-OfficialMode {
    ...
    Write-Info '菜单 1 阶段：准备 official profile。'
    Prepare-OfficialProfileForOfficialMode -NoWrite:$NoLaunch | Out-Null
    ...
    Restore-CodexUiStateSnapshotUnlessRecentRepair -NoWrite:$NoLaunch | Out-Null
...
function Start-ThirdPartyPreserveAuthMode {
    ...
    $officialAuthBaseline = New-OfficialAuthBaselineForPreserveMode -NoWrite:$NoLaunch
    Save-OfficialAuthOnlyIfCurrentLooksOfficial -NoWrite:$NoLaunch | Out-Null
    ...
    if (-not (Ensure-OfficialAuthForPreserveMode -Baseline $officialAuthBaseline -NoWrite:$NoLaunch)) {
    ...
    if (-not (Confirm-OfficialAuthForPreserveMode -Baseline $officialAuthBaseline -NoLaunch:$NoLaunch)) {
        return
    }
    ...
    Write-Warn '菜单 2 跳过恢复旧 Codex UI 快照，避免把已出现过的错误页状态重新写回。界面偏好已在切换前保存为诊断备份。'
...
function Start-RepairMode {
    ...
    Write-Info '修复 Codex Desktop 启动错误页：关闭 Codex，备份并清理 Electron/UI 崩溃缓存，然后重新启动。'
    Write-Next '此修复不会删除 auth.json、config.toml、聊天记录、profiles 或 CCSwitch 数据。'
    ...
    $codexClosed = Stop-LauncherProcess -DisplayName 'Codex' -PreferredPath $codexProcessPath -FallbackNames $Script:CodexProcessNames -TimeoutSeconds 5 -ForceImmediately -NoLaunch:$NoLaunch
    if (-not $codexClosed) {
        Write-ErrorLine 'Codex 没有完全退出，已停止修复，避免边运行边清理缓存。'
        Write-Next '请手动关闭 Codex 后再运行 repair。'
        return
    }

    $backupRoot = Backup-CodexDesktopCrashState -NoWrite:$NoLaunch
    Export-CodexDesktopRepairEvidence -DestinationDir $backupRoot -NoWrite:$NoLaunch | Out-Null
    Reset-LauncherUiSnapshotAfterRepair -EvidenceDir $backupRoot -NoWrite:$NoLaunch
    Write-RepairMarker -EvidenceDir $backupRoot -NoWrite:$NoLaunch
    Clear-CodexDesktopCrashState -NoWrite:$NoLaunch | Out-Null

    Start-LaunchTarget -Target $codexTarget -SetEnv @{} -RemoveEnv @('CODEX_HOME') -NoLaunch:$NoLaunch
    Write-Next "如修复后仍看到错误页，请发送本次日志文件：$Script:RunLogPath"
}
...
function Show-Menu {
    ...
    while ($true) {
        ...
        Write-Host '4. 修复 Codex Desktop 启动错误页'
        Write-Host '   - 备份并清理 Electron/UI 崩溃缓存，不删除登录、配置和聊天。'
        Write-Host '5. 诊断/体检当前状态'
        Write-Host '   - 只读检查，不修改任何文件。'
        Write-Host '6. 初始化/升级本机启动器'
        ...
        switch ($choice) {
            ...
            '4' { Start-RepairMode -Config $Config -NoLaunch:$NoLaunch; return }
            '5' { Invoke-Doctor -Config (Read-LauncherConfig); if ($NoLaunch) { return } }
            '6' { Invoke-Bootstrap -Config $Config -NoLaunch:$NoLaunch; return }
            '7' { return }
            default { Write-Warn '请输入 1、2、3、4、5、6 或 7。' }
        }
    }
}
...
switch ($Mode) {
    ...
    'repair' { Start-RepairMode -Config $config -NoLaunch:$NoLaunch }
